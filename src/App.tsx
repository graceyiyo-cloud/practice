...
export default function App(){
 ...
 useEffect(()=>{
  ...
  const handleBack=(event:PopStateEvent)=>{
   const state=event.state as {littleWords?:boolean;screen?:NavScreen;groupId?:string}|null;
   const screen=state?.littleWords&&state.screen?state.screen:"practice";
   setAnswerFocused(false);
   input.current?.blur();
   if(screen==="quiz"){setTab("practice");setReviewGroupId(null);return}
   if(screen==="review"){setTab("groups");setReviewGroupId(state?.groupId||null);setQuiz([]);setDone(false);return}
   setTab(screen);
   setReviewGroupId(null);
   if(screen==="practice"){setQuiz([]);setDone(false)}
  };
 ...
 const fetchBopomofo=async(text:string)=>{const r=await fetch(`https://www.moedict.tw/a/${encodeURIComponent(text)}.json`);if(!r.ok)return "";const d=await r.json();return (d.h?.[0]?.b||"") as string};
 const searchWord=async()=>{const q=zh.trim();if(!q)return;setLookup("loading");const local=dictionary.find(x=>x[0]===q);if(local){setBpmf(local[2]);setLookup("found");return}try{let pronunciation=await fetchBopomofo(q);if(!pronunciation&&[...q].length>1){const parts=await Promise.all([...q].map(fetchBopomofo));if(parts.every(Boolean))pronunciation=parts.join(" ")}if(pronunciation){setBpmf(pronunciation);setLookup("found")}else setLookup("missing")}catch{setLookup("missing")}};
 const pushScreen=(screen:NavScreen,groupId?:string)=>window.history.pushState({littleWords:true,screen,groupId},"");
 const changeTab=(nextTab:Tab)=>{pushScreen(nextTab);setTab(nextTab);setReviewGroupId(null)};
 const start=()=>{if(!pool.length)return;const q=[...pool].sort(()=>Math.random()-.5);pushScreen("quiz");setQuiz(q);setIdx(0);setScore(0);setAnswer("");setResult("idle");setAttempts(0);setDone(false);setTimeout(()=>speak(q[0].en),150)};
 ...
 return <main><Header tab={tab} setTab={changeTab}/><div className="shell">
  ...
  :<section className={`quiz card${answerFocused?" answering":""}`}><div className="qtop"><span>第 {idx+1} / {quiz.length} 題</span><span>答對 {score} 題</span></div><div className="progress"><i style={{width:`${(idx+1)/quiz.length*100}%`}}/></div><p>看中文，寫出完整英文單字</p><div className="zh">{cur?.zh}</div><div className="bpmf">{cur?.bpmf}</div><button className="speak" onClick={()=>speak()}>🔊 再聽一次</button><form onSubmit={check}><input ref={input} autoFocus value={answer} onFocus={()=>setAnswerFocused(true)} onBlur={()=>setAnswerFocused(false)} onChange={e=>{setAnswer(e.target.value);if(result==="wrong")setResult("idle")}} placeholder="在這裡輸入英文"/>{result==="idle"&&<button type="button" className="primary" onPointerDown={e=>{e.preventDefault();e.currentTarget.form?.requestSubmit()}} onClick={e=>{if(e.detail===0)e.currentTarget.form?.requestSubmit()}}>確定</button>}</form>{result==="correct"&&<div className="feedback good"><b>答對了！</b><span>{cur?.en}</span><button onClick={next}>下一題 →</button></div>}{result==="wrong"&&<div className="feedback try"><b>再想想看～</b><span>{attempts<2?"輸入錯誤，請再試一次！":"可以再聽一次或修改答案。"}</span>{attempts>=2&&<button type="button" onClick={()=>{input.current?.blur();setAnswer(cur?.en || "");setResult("correct")}}>看看答案</button>}</div>}</section>)}
  {tab==="words"&&<section><Title en="MY WORDS" title="我的單字" p="輸入中文後查詢注音，英文請自行輸入。"/><div className="cols"><form className="card editor" onSubmit={saveWord}><h3>＋ 新增單字</h3>
   <label>中文<div className="searchrow"><input value={zh} onChange={e=>{setZh(e.target.value);setBpmf("");setLookup("idle")}} onKeyDown={e=>{if(e.key==="Enter"){e.preventDefault();searchWord()}}} placeholder="例如：蘋果"/><button type="button" className="searchbtn" onClick={searchWord} disabled={!zh.trim()||lookup==="loading"}>{lookup==="loading"?"查詢中…":"查注音"}</button></div>{lookup==="found"&&<small className="lookupok">已帶出注音</small>}{lookup==="missing"&&<small className="lookuperror">查不到注音，請手動輸入。</small>}</label>
   <label>注音<input className="bpmf" value={bpmf} onChange={e=>setBpmf(e.target.value)} placeholder="搜尋後自動帶出"/></label>
   <label>英文（自行輸入）<div className="englishsearch"><div className="with"><input value={en} onChange={e=>setEn(e.target.value)} placeholder="例如：apple"/><button type="button" onClick={()=>speak(en)} aria-label="播放英文發音"><Volume2 size={17}/></button></div></div></label>
   ...
   <button className="primary" disabled={groups.length===0}>儲存單字</button>{groups.length===0&&<div style={{color:'var(--coral)',fontSize:13,marginTop:12}}>請先新增分類才能儲存單字</div>}</form><WordList words={words} groups={groups} speak={speak} move={(id,group)=>setWords(s=>s.map(w=>w.id===id?{...w,group}:w))} remove={id=>setWords(s=>s.filter(w=>w.id!==id))}/></div></section>}
  {tab==="groups"&&(reviewGroupId&&groups.some(g=>g.id===reviewGroupId)?<GroupReview group={groups.find(g=>g.id===reviewGroupId)!} words={words.filter(w=>w.group===reviewGroupId)} speak={speak} back={()=>window.history.back()} practice={()=>{setSelected([reviewGroupId]);setQuiz([]);setDone(false);setReviewGroupId(null);pushScreen("practice");setTab("practice")}}/>:<section><Title en="WORD GROUPS" title="單字分組" p="點選分類可查看單字並先自行複習。"/><form className="addgroup card" onSubmit={e=>{e.preventDefault();if(createGroup(ng))setNg("")}}><input value={ng} onChange={e=>setNg(e.target.value)} placeholder="輸入新分組名稱"/><button className="primary">＋ 新增分組</button></form><div className="groupgrid">{groups.map(g=><article className="card groupcard" key={g.id} style={{"--tone":g.color} as React.CSSProperties}><button className="group-open" onClick={()=>{pushScreen("review",g.id);setReviewGroupId(g.id)}}><span>▰</span><div><h3>{g.name}</h3><p>{words.filter(w=>w.group===g.id).length} 個單字 · 點選查看</p></div><ArrowRight size={20}/></button><button className="group-delete" aria-label={`刪除${g.name}分類`} disabled={groups.length<=1} onClick={()=>{const f=groups.find(x=>x.id!==g.id);if(!f)return;setWords(s=>s.map(w=>w.group===g.id?{...w,group:f.id}:w));setGroups(s=>s.filter(x=>x.id!==g.id));setWg(f.id)}}>×</button></article>)}</div></section>)}
...
function WordList({words,groups,speak,move,remove}:{words:Word[];groups:Group[];speak:(s:string)=>void;move:(id:string,group:string)=>void;remove:(id:string)=>void}){return <div className="wordlist card"><div className="listhead"><h3>全部單字</h3><span>{words.length} 個</span></div>{words.map(w=><article key={w.id}><button className="sound" aria-label={`播放${w.en}發音`} onClick={()=>speak(w.en)}><Volume2 size={18}/></button><div><b>{w.en}</b><span>{w.zh} <small className="bpmf">{w.bpmf}</small></span></div><select className="word-group-select" aria-label={`移動${w.en}到其他分類`} value={w.group} onChange={e=>move(w.id,e.target.value)}>{groups.map(g=><option key={g.id} value={g.id}>{g.name}</option>)}</select><button className="delete" aria-label={`刪除${w.en}`} onClick={()=>remove(w.id)}>×</button></article>)}</div>}
function GroupReview({group,words,speak,back,practice}:{group:Group;words:Word[];speak:(s:string)=>void;back:()=>void;practice:()=>void}){return <section className="group-review"><button className="back-button" onClick={back}><ArrowLeft size={18}/>返回分組</button><div className="review-heading"><div><span className="eyebrow">GROUP REVIEW</span><h2>{group.name}</h2><p>先看看中文與英文，也可以播放發音自行複習。</p></div><button className="primary" disabled={!words.length} onClick={practice}>前往測驗<ArrowRight size={18}/></button></div>{words.length===0?<div className="card review-empty">這個分類還沒有單字</div>:<div className="review-grid">{words.map(w=><article className="card review-word" key={w.id} style={{"--tone":group.color} as React.CSSProperties}><button className="review-sound" aria-label={`播放${w.en}發音`} onClick={()=>speak(w.en)}><Volume2 size={20}/></button><div><b>{w.en}</b><span>{w.zh}</span><small className="bpmf">{w.bpmf}</small></div></article>)}</div>}</section>}
