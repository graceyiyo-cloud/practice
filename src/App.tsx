...
export default function App(){
 ...
 return <main><Header tab={tab} setTab={setTab}/><div className="shell">
  ...
  {tab==="groups"&&(reviewGroupId&&groups.some(g=>g.id===reviewGroupId)?<GroupReview group={groups.find(g=>g.id===reviewGroupId)!} words={words.filter(w=>w.group===reviewGroupId)} speak={speak} back={()=>setReviewGroupId(null)} practice={()=>{setSelected([reviewGroupId]);setQuiz([]);setDone(false);setReviewGroupId(null);setTab("practice")}}/>:<section><Title en="WORD GROUPS" title="單字分組" p="點選分類可查看單字並先自行複習。"/><form className="addgroup card" onSubmit={e=>{e.preventDefault();if(createGroup(ng))setNg("")}}><input value={ng} onChange={e=>setNg(e.target.value)} placeholder="輸入新分組名稱"/><button className="primary">＋ 新增分組</button></form><div className="groupgrid">{groups.map(g=><article className="card groupcard" key={g.id} style={{"--tone":g.color} as React.CSSProperties}><button className="group-open" onClick={()=>setReviewGroupId(g.id)}><span>▰</span><div><h3>{g.name}</h3><p>{words.filter(w=>w.group===g.id).length} 個單字 · 點選查看</p></div><ArrowRight size={20}/></button><button className="group-delete" aria-label={`刪除${g.name}分類`} disabled={groups.length<=1} onClick={()=>{const f=groups.find(x=>x.id!==g.id);if(!f)return;setWords(s=>s.map(w=>w.group===g.id?{...w,group:f.id}:w));setGroups(s=>s.filter(x=>x.id!==g.id));setWg(f.id)}}>×</button></article>)}</div></section>)}
